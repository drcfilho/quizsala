...
## T24 · Ciclo completo da tela.php — cronômetro, explicação, resumo final e timeout do aluno *(concluída)*

**Não estava no plano original — pedido do usuário descrevendo o fluxo completo que esperava ver na `tela.php`, do "aguardando o início" até o resumo final, comparado com o que já estava implementado.**

**Entrega:** quatro pedaços novos/mudados no ciclo, confirmados com o usuário antes de implementar (via `AskUserQuestion`): cronômetro opcional por questão (duração fixa, só avisa ao zerar — a revelação continua sempre manual, D6); explicação da resposta certa visível no projetor na revelação (campo já existia desde T09c, só pro professor); resumo por questão na tela `encerrada` (participantes+questões no topo, acertos/erros/não-responderam + as mesmas barras da revelação, por questão); e a tela final do aluno avança sozinha pro "Obrigado por participar!" depois de 5min sem interação. **Confirmado explicitamente que NÃO muda:** a contagem por alternativa continua invisível antes da revelação (D7/T03, travado); "passar para outra prova" não ganhou tela nova, continua sendo o fluxo de Nova sessão já existente.

**Arquivos:** `db/schema.sql`, `public/api/comando.php`, `src/util.php`, `public/api/painel.php`, `public/admin/questao.php`, `public/assets/tela.js`/`tela.css`, `public/assets/admin.js`/`admin.css`, `public/assets/aluno.js`, `bin/teste.sh`

**Passos**
1. ~~Schema~~ Feito — `questoes.duracao_segundos` (nullable, `NULL`/`0` = sem cronômetro) e `sessoes.fase_iniciada_em` (carimbo de quando a fase atual começou, base do cronômetro calculada no servidor — projetor e celular do professor concordam mesmo que um dos dois recarregue no meio). `comando.php` grava esse carimbo em toda transição.
2. ~~Cronômetro~~ Feito. Ponto crítico de design (achado por um agente de planejamento dedicado antes de codar): um `setInterval` ingênuo de 1s atropelaria a otimização do T21 (`chaveAtual`, redesenho só quando a questão muda de verdade). Solução: `tela.js`/`admin.js` (duplicado localmente, cada `.js` deste projeto é autocontido) têm um `setInterval` local de 1s que só reescreve o texto/classe do elemento já existente, recalibrado a partir do valor do servidor a cada poll de 2s — ritmo visual e ritmo de rede desacoplados. Ao chegar a zero: pára o próprio intervalo, troca o *texto* do rótulo pra "Tempo esgotado" (não só a cor da borda — Regra do Sinal Duplo). No admin, o botão Revelar ganha o mesmo destaque de "todos responderam" quando o tempo esgota — só sinaliza, nunca dispara sozinho (D6).
3. ~~Explicação no projetor~~ Feito — `painel.php` inclui `explicacao` no payload de `revelado` só quando preenchida (mesmo idioma de campo opcional que `distribuicao`/D7 já usa); `renderizarResultado()` mostra um bloco com borda esquerda grossa, sem cor nova.
4. ~~Resumo final~~ Feito — nova `resumoSessaoEncerrada()` em `util.php` (reaproveita `distribuicaoQuestao()`, extraída do que já existia inline em `painel.php`), exposta em `painel.php` quando `fase=encerrada`. **Achado do agente de planejamento, corrigido antes de virar bug:** a tela `encerrada` nunca tem `dados.questao`, então a chave de dirty-check do T21 sempre calculava `null` ali — sem ajuste, o resumo (barras animadas incluídas) seria redesenhado do zero a cada poll de 2s, pra sempre. `chaveAtual` em `tela.js` passou a valer `'encerrada'` nesse caso, com o mesmo short-circuit "nada mudou, não redesenha" que `revelado` já tinha. Confirmado manualmente no navegador: o nó das barras se manteve o mesmo (sem reset de largura) em 2+ polls consecutivos na tela de resumo.
5. ~~Timeout do aluno~~ Feito — `renderizarPlacar()` arma um `setTimeout` de 5min pra `renderizarAgradecimento()`; os dois botões existentes ("Salvar comprovante"/"Concluir") cancelam esse timeout antes de agir. Comportamento 100% local, sem mudança de servidor.

**Como testar** `bash bin/teste.sh` — 96/96 (21 checks novos no Caso 38, campo de cronômetro no editor salvando/limpando, e Caso 39, uma sessão isolada e descartável cobrindo `fase_iniciada_em`, `temporizador` presente/ausente/nunca-negativo, `explicacao` presente/ausente e o resumo completo — sem reaproveitar `AULA01`/`CODIGO2`, que já acumulam estado de casos anteriores). Ciclo completo testado manualmente no navegador (3 abas: projetor, controle do professor, aluno): aguardando com QR → Iniciar → cronômetro contando em sincronia nas duas telas → tempo esgota, mostra "Tempo esgotado" em vermelho e destaca Revelar nos dois sinais (completo + esgotado) → Revelar mostra barras + explicação → Próxima (questão sem cronômetro, campo some) → responde errado, revela, mostra explicação da segunda questão → Próxima na última questão (sem responder, testando "não respondeu") → Encerrar automático mostra o resumo com contagens certas por questão → aluno vê placar 1/3, clica Concluir, vê "Obrigado por participar!". Zero erros no console em todas as três abas. **T-D (timeout de 5min) não tem cobertura em `teste.sh` de propósito** — é `setTimeout`/DOM puro sem efeito colateral observável por `curl`, sem runner de JS headless no projeto (mesmo padrão já usado pra T04/T14: implementado, validado manualmente, sem teste fabricado que não testaria nada de verdade); a espera de 5min completa não foi reproduzida no navegador (o clique em "Concluir" já confirma que `limparTimeoutPlacar()` roda sem erro no caminho normal).

**Pronto quando** os cinco pontos acima estão implementados, `bin/teste.sh` passa 100%, e o ciclo completo (aguardando → espera → respondendo com cronômetro → revelado com explicação → loop → encerrada com resumo → placar do aluno) funciona sem erro de console. **Confirmado.**
